...
public function similar($id_vehicle)
{
  $builder = $this->db->table("tblvehicles");
  
  $builder->select('tblvehicles.*,tblvehicles.VehiclesTitle,tblbrands.Brands,tblvehicles.PricePerDay,tblvehicles.FuelType,tblvehicles.ModelYear,tblvehicles.id_vehicle,tblvehicles.SeatingCapacity,tblvehicles.VehiclesOverview,tblvehicles.Vimage1');
  // $builder->from('tblvehicles');
  $builder->join('tblbrands', 'tblbrands.id=tblvehicles.VehiclesBrand');
  $builder->where('tblvehicles.VehiclesBrand',$id_vehicle);
 $query=$builder->get();
 return $query->getResult();
